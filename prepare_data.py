...
def my_anti_shuffle(input_image, ratio):
    ...
    :param input_image: [18， 18， 3]
    :param ratio: 2
    :return: anti_shuffle:[9, 9, 12]
    """
...
def prepare_images(params):
    ...
    for root, dirnames, filenames in os.walk(params['image_dir']):
        for filename in filenames:
            ...
            hr_image = misc.imread(path)    # 读原始图像
            height = hr_image.shape[0]      # 原始图像的高
            new_height = height - height % ratio    # 裁剪，使高变成radio的整数
            width = hr_image.shape[1]
            new_width = width - width % ratio     # 与高同理
            hr_image = hr_image[0:new_height,0:new_width]      # 得到边长是radio整数倍的原图像
            blurred = ndimage.gaussian_filter(hr_image, sigma=(1, 1, 0))    # 高斯模糊
            lr_image = blurred[::ratio,::ratio,:]     # 按照radio倍数抽取原图像，得到低分辨率图像
            # 注意此时hr图像的分辨率是lr的radio倍

            height = hr_image.shape[0]
            width = hr_image.shape[1]
            vertical_number = height / hr_stride - 1    # 值为hr_stride在height的最大整数倍
...
def prepare_data(params):
    ...
    # hr_start_idx？
    hr_start_idx = ratio * edge / 2  # 8
    hr_end_idx = hr_start_idx + (lr_size - edge) * ratio    # 26
    sub_hr_size = (lr_size - edge) * ratio   # 18

    for dir_idx, image_dir in enumerate(image_dirs):    # 这个函数返回数据下标和数据
        ...
        for root, dirnames, filenames in os.walk(image_dir + "/lr"):
            for filename in filenames:
                ...
                # 这里不太明白，为什么lr_data是[17,17]，而hr_data是[18,18],但是感觉关系也不大
